...
public class ThemeSearchFunctionTests(PostgresFixture postgres) : IClassFixture<PostgresFixture>, IAsyncLifetime
{
    ...
    private async Task SeedStrengthAsync()
    {
        await using var connection = await _dataSource.OpenConnectionAsync();
        await connection.ExecuteAsync(
            ...
            "INSERT INTO dw.dim_subject (subject_name, subject_code) VALUES ('Inscrição indevida', 1001), ('Negativação indevida', 1002); " +
            "INSERT INTO dw.bridge_theme_subject (theme_sk, subject_sk) " +
            "SELECT t.theme_sk, s.subject_sk FROM dw.dim_theme t JOIN dw.dim_subject s ON s.subject_code = CASE " +
            "WHEN t.theme_name = @wrongfulListing THEN 1001 WHEN t.theme_name = @wrongfulNegativeRecord THEN 1002 END " +
            "WHERE t.theme_name IN (@wrongfulListing, @wrongfulNegativeRecord); " +
            "INSERT INTO dw.dim_case (case_number, court_sk, case_class_sk, court_level, source, extracted_at) " +
            "SELECT 'SEARCH-' || t.theme_key || '-' || series.case_index, c.court_sk, cc.case_class_sk, 'First', 'datajud', now() " +
            "FROM dw.dim_theme t CROSS JOIN LATERAL generate_series(1, CASE WHEN t.theme_name = @wrongfulListing THEN 2 ELSE 10 END) AS series(case_index) " +
            ...
            "ON s.subject_code = CASE WHEN split_part(c.case_number, '-', 2)::bigint = " +
            "(SELECT theme_key FROM dw.dim_theme WHERE theme_name = @wrongfulListing) THEN 1001 ELSE 1002 END " +
            "WHERE c.case_number LIKE 'SEARCH-%'; " +
            "INSERT INTO dw.fact_case_event (case_sk, court_sk, movement_sk, date_sk, occurred_at, source_url, extracted_at, natural_key) " +
            "SELECT c.case_sk, c.court_sk, m.movement_sk, 20250615, TIMESTAMPTZ '2025-06-15', " +
            "'https://example.org', now(), 'search:' || c.case_sk FROM dw.dim_case c JOIN dw.dim_movement m ON m.movement_code = CASE " +
            "WHEN split_part(c.case_number, '-', 2)::bigint = (SELECT theme_key FROM dw.dim_theme WHERE theme_name = @wrongfulListing) " +
            "AND split_part(c.case_number, '-', 3) = '2' THEN 220 ELSE 219 END WHERE c.case_number LIKE 'SEARCH-%'; " +
            ...
            new { wrongfulListing = WrongfulListing, wrongfulNegativeRecord = WrongfulNegativeRecord });
    }
    ...
    [Fact]
    public async Task Orders_matching_themes_by_strength_before_applying_the_limit()
    {
        var results = await SearchAsync("inscricao indevida");
        var limitedResults = await SearchAsync("inscricao indevida", 1);
        ...
        Assert.Equal([WrongfulNegativeRecord], limitedResults.Select(result => result.ThemeName));
    }
